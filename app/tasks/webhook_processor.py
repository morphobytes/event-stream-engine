...
from app.core.data_model import InboundEvent, DeliveryReceipt, Message, User, ConsentState, MessageStatus
import re
import json
    ...
    with app.app_context():
        try:
            ...
            if message:
                ...
                if message.status == MessageStatus.SENT and old_status != MessageStatus.SENT:
                    message.sent_at = datetime.utcnow()
                elif message.status == MessageStatus.DELIVERED and old_status != MessageStatus.DELIVERED:
                ...
                print(f"Updated message {message.id} status: {old_status} -> {message.status}")
            else:
