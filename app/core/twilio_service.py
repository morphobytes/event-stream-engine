...
class TwilioService:
    ...
    def send_message(
        self, to_phone: str, message_content: str, channel: str = "whatsapp"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            message = self.client.messages.create(
                body=message_content, from_=from_phone, to=to_phone_formatted
            )
    ...
    def get_message_status(self, message_sid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch message status for {message_sid}: {e.code} - {e.msg}"
            )

            return {"success": False, "error_code": str(e.code), "error_message": e.msg}

        except Exception as e:
            logger.error(
                f"Unexpected error fetching status for {message_sid}: {str(e)}"
            )
