...
from sqlalchemy import Column, Integer, String, Boolean, DateTime, ForeignKey, Text, UniqueConstraint, Index, Enum
...
class User(db.Model):
    ...
    phone_e164 = Column(String(16), primary_key=True, comment="User's phone number in E.164 format (PK).")
    
    # Attributes JSON - For personalization and segmentation
    attributes = Column(JSONB, default={}, comment="JSON object for custom user attributes.")
    
    # Consent State - Crucial for compliance and STOP command enforcement
    consent_state = Column(Enum(ConsentState), default=ConsentState.OPT_IN, comment="Current messaging consent state (OPT_IN/OPT_OUT/STOP).")
...
class Subscription(db.Model):
    ...
    topic = Column(String(100), nullable=False, comment="Messaging topic the user is subscribed to.")
    
    __table_args__ = (
        PrimaryKeyConstraint('user_phone', 'topic'),
    )
...
class Message(db.Model):
    ...
    id = Column(Text, primary_key=True, default=lambda: str(uuid.uuid4()), comment="Internal unique message ID (UUID).")
    
    campaign_id = Column(Integer, ForeignKey('campaigns.id'), nullable=False)
    recipient_phone = Column(String(16), ForeignKey('users.phone_e164'), nullable=False)
    
    # State Machine Core
    status = Column(Enum(MessageStatus), default=MessageStatus.QUEUED, comment="Current message status (QUEUED, SENT, DELIVERED, FAILED, etc.).")
    
    # Audit & Provider Fields
    provider_sid = Column(String(50), index=True, unique=True, comment="Twilio MessageSID for callback tracking.")
    error_code = Column(Integer, nullable=True, comment="Twilio ErrorCode on FAILED or UNDELIVERED status.")
...
class Campaign(db.Model):
    ...
    topic = Column(String(100), nullable=False, comment="The topic this campaign targets (e.g., 'price_alert').")
    ...
    status = Column(String(50), default="DRAFT", comment="Campaign status (DRAFT, READY, RUNNING, COMPLETED).")
    
    # Outbound Logic Rules
    rate_limit_per_second = Column(Integer, default=10, comment="Max messages to send per second.")
    quiet_hours_start = Column(String(5), comment="Time (e.g., '22:00') to pause sending.")
    quiet_hours_end = Column(String(5), comment="Time (e.g., '08:00') to resume sending.")
...
class Template(db.Model):
    ...
    id = Column(Integer, primary_key=True)
    name = Column(String(100), unique=True, nullable=False, comment="Internal name for the template.")
    channel = Column(String(20), default="whatsapp", comment="Messaging channel (always 'whatsapp' here).")
    locale = Column(String(10), default="en_US", comment="Language/locale for the message.")
...
class Segment(db.Model):
    ...
    definition_json = Column(JSONB, comment="JSON/DSL defining user filtering rules (e.g., {'attribute': 'city', 'value': 'Colombo'}).")    
    created_at = Column(DateTime, default=datetime.utcnow)
...
class InboundEvent(db.Model):
    ...
    raw_payload = Column(JSONB, nullable=False, comment="Full, original JSON payload from Twilio.")
    
    # Normalized fields (for easy querying)
    message_sid = Column(String(50), index=True, comment="Twilio MessageSid.")
    from_phone = Column(String(16), index=True, comment="E.164 number that sent the message (From).")
    channel_type = Column(String(20), index=True, comment="Channel type extracted from Twilio prefix (whatsapp, sms, messenger).")
    normalized_body = Column(Text, comment="Cleaned and lowercased message body.")
    processed_at = Column(DateTime, default=datetime.utcnow)
    
    # Relationship columns
    message_id = Column(Text, ForeignKey('messages.id'), nullable=True, comment="Link to message if this was a reply.")
    user_phone = Column(String(16), ForeignKey('users.phone_e164'), nullable=True, comment="Link to user who sent the message.")
...
class DeliveryReceipt(db.Model):
    ...
    id = Column(Text, primary_key=True, default=lambda: str(uuid.uuid4()))
    raw_payload = Column(JSONB, nullable=False, comment="Full, original JSON payload from Twilio status callback.")
    
    # Normalized fields (for easy querying)
    message_sid = Column(String(50), index=True, comment="Twilio MessageSid of the message being updated.")
    message_status = Column(String(20), index=True, comment="Final status (e.g., delivered, failed).")
    error_code = Column(Integer, comment="Error code if delivery failed.")
    received_at = Column(DateTime, default=datetime.utcnow)
    
    # Foreign Keys for proper relationships
    message_id = Column(Text, ForeignKey('messages.id'), nullable=True, comment="Link to the message record if available.")
    user_phone = Column(String(16), ForeignKey('users.phone_e164'), nullable=True, comment="Link to user for aggregated reporting.")
